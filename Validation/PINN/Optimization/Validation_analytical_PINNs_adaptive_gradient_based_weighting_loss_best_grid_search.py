...
# ============================================================
class PINNSolver:
    def __init__(self, M_values, Gamma_values, layers, activation_function, optimizer_name, output_dir, chi_min=0.0, N=10000, epochs=50000, learning_rate=1e-5, tol=1e-5, init_seed=42, snapshot_every=1000, error_eval_points=5000, additional_epochs_after_tolerance=5000):
        super().__init__()
        ...
        # (NEW) weight history for adaptive gradient-based weighting
        self.weight_history = {device: [] for device in self.devices}
    ...
    def save_weight_history_to_csv(self):
        """Save weight history to CSV file"""
        for device in self.devices:
            if len(self.weight_history[device]) > 0:
                # Convert weight history to DataFrame
                weight_data = []
                for weight_record in self.weight_history[device]:
                    weight_data.append({
                        'Epoch': weight_record['epoch'],
                        'ODE_Weight': weight_record['ode_weight'],
                        'Boundary_Weight': weight_record['boundary_weight'],
                        'Integral_Weight': weight_record['integral_weight']
                    })
                
                df_weights = pd.DataFrame(weight_data)
                
                # Save to CSV
                weights_csv_path = os.path.join(self.output_dir, "weights_summary_validation_PINN_adaptive_gradient_based_weighting_loss_best_grid_search.csv")
                df_weights.to_csv(weights_csv_path, index=False)
                print(f"Saved weight history to {weights_csv_path}")
                break  # Only save once (same data for all devices)
    ...
    def save_summary(self):
        ...
        with open(summary_filename, "w", encoding='utf-8') as f:
            f.write("\n============================== SUMMARY REPORT ==============================\n")
            f.write("| Device |   M   |  Γ  |  χ_min  |  χ_max  | χ_max(Okwen) | Integral | Final Total Loss | Final ODE Loss | Final Boundary Loss | Final Integral Loss | Final Err(L2) | Epochs | Time(s) | Tol Reached | Early Stop | Final ODE Weight | Final BC Weight | Final Integral Weight |\n")
            f.write("|--------|-------|-----|---------|---------|-------------|----------|------------------|----------------|---------------------|---------------------|---------------|--------|---------|-------------|------------|------------------|-----------------|----------------------|\n")
            ...
            for device in self.devices:
                for i, (M, Gamma) in enumerate(zip(self.M_values, self.Gamma_values)):
                    ...
                    final_total_loss = self.loss_history[device][i][-1] if len(self.loss_history[device][i]) else float('nan')
                    final_ode_loss = self.ode_loss_history[device][i][-1] if len(self.ode_loss_history[device][i]) else float('nan')
                    final_boundary_loss = self.boundary_loss_history[device][i][-1] if len(self.boundary_loss_history[device][i]) else float('nan')
                    final_integral_loss = self.integral_loss_history[device][i][-1] if len(self.integral_loss_history[device][i]) else float('nan')
                    ...
                    # Get final weights
                    final_ode_weight = self.loss_weights['ode']
                    final_boundary_weight = self.loss_weights['boundary']
                    final_integral_weight = self.loss_weights['integral']

                    f.write(f"| {str(device):6s} | {M:5.2f} | {Gamma:3.2f} | {self.chi_min:7.3f} | {chi_max:7.3f} |"
                            f" {chi_max_ok:11.3f} | {integral_val:8.4f} | {final_total_loss:14.2e} | {final_ode_loss:12.2e} |"
                            f" {final_boundary_loss:17.2e} | {final_integral_loss:17.2e} | {final_err:11.2e} |"
                            f" {epochs_ran:6d} | {tsec:7.2f} | {tol_reached:11s} | {early_stop:10s} |"
                            f" {final_ode_weight:14.3f} | {final_boundary_weight:13.3f} | {final_integral_weight:18.3f} |\n")
    ...
    def train(self):
            ...
            for epoch in range(1, self.epochs + 1):
                for i, (M, Gamma) in enumerate(zip(self.M_values, self.Gamma_values)):
                    ...
                    if epoch % self.weight_update_frequency == 0:
                        ...
                        # Record weight history (every time weights are updated)
                        self.weight_history[device].append({
                            'epoch': epoch,
                            'ode_weight': self.loss_weights['ode'],
                            'boundary_weight': self.loss_weights['boundary'],
                            'integral_weight': self.loss_weights['integral']
                        })
        ...
        # Save weight history to CSV
        self.save_weight_history_to_csv()
...
def create_summary_dataframe(main_output_dir):
    ...
    for run_dir in os.listdir(main_output_dir):
        ...
        # Also check if summary file exists in subdirectories (common case)
        if not os.path.exists(summary_path):
            # Look for the summary file in any subdirectory
            for subitem in os.listdir(full_path):
                subpath = os.path.join(full_path, subitem)
                if os.path.isdir(subpath):
                    potential_summary = os.path.join(subpath, "training_summary_validation.txt")
        ...
        for line in lines:
            if line.startswith("|") and ("cuda" in line or "cpu" in line) and "Device" not in line:
                ...
                try:
                    ...
                    final_total_loss = float(cols[7])
                    final_ode_loss = float(cols[8])
                    final_boundary_loss = float(cols[9])
                    final_integral_loss = float(cols[10])
                    final_err = float(cols[11])
                    epochs_ran = int(cols[12])
                    time_s = float(cols[13])
                    # Skip cols[14] and cols[15] (Tol Reached and Early Stop - they contain "N/A")
                    final_ode_weight = float(cols[16])  # Final ODE Weight
                    final_boundary_weight = float(cols[17])  # Final BC Weight  
                    final_integral_weight = float(cols[18])  # Final Integral Weight
                    ...
                    results.append({
                        ...
                        "Final ODE Loss": final_ode_loss,
                        "Final Boundary Loss": final_boundary_loss,
                        "Final Integral Loss": final_integral_loss,
                        "Final Err L2": final_err,
                        "Epochs": epochs_ran,
                        "Training Time (s)": time_s,
                        "Final ODE Weight": final_ode_weight,
                        "Final BC Weight": final_boundary_weight,
                        "Final Integral Weight": final_integral_weight
                    })
                except Exception:
                    continue
...
if __name__ == "__main__":
    ...
    for layers, activation in itertools.product(layer_options, activation_functions):
        ...
        run_dir = f"run_layers_{layers_str}_act_{activation}"
        full_run_dir = os.path.join(main_output_dir, run_dir)

        print("\n====================================================")
        print(f"RUN: layers={layers}, activation={activation}")
        print("====================================================")
        ...
        try:
            ...
            # score config by average FINAL loss across both pairs
            ...
            print(f"FAILED config layers={layers}, activation={activation} | error: {e}")
    ...
    df.to_csv(os.path.join(main_output_dir, "results_summary_validation_PINN_adaptive_gradient_based_weighting_loss_best_grid_search.csv"), index=False)
    ...
    print("Saved CSV:", os.path.join(main_output_dir, "results_summary_validation_PINN_adaptive_gradient_based_weighting_loss_best_grid_search.csv"))
